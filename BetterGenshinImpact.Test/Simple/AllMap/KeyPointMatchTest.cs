...
public class KeyPointMatchTest
{
    ...
    {
        // var trainMat = new Mat(@"E:\HuiTask\Улучшенный Genshin Impact\сопоставление карт\Сравнивать\Наложение\Невозможно сопоставить2.png", ImreadModes.Color);
        var trainMat = new Mat(@"E:\HuiTask\Улучшенный Genshin Impact\сопоставление карт\Сравнивать\мини-карта\Clip_20240323_152015.png", ImreadModes.Grayscale);

        // tar = tar.Resize(new Size(tar.Width * 2, tar.Height * 2), 0, 0, InterpolationFlags.Nearest);

        // var queryMat = new Mat(@"E:\HuiTask\Улучшенный Genshin Impact\сопоставление карт\combined_image_lim[quick].png", ImreadModes.Color);
        var queryMat = new Mat(@"E:\HuiTask\Улучшенный Genshin Impact\сопоставление карт\combined_image_2048_lim[quick].png", ImreadModes.Grayscale);
        var res = MatchPicBySurf(trainMat, queryMat);

        Cv2.ImWrite(@"E:\HuiTask\Улучшенный Genshin Impact\сопоставление карт\s1.png", res);
    }
    ...
    /*public static Mat MatchPicBySift(Mat matSrc, Mat matTo)
    {
        ...
        {
            var kpPath = @"E:\HuiTask\Улучшенный Genshin Impact\сопоставление карт\sift.kp";
            var kpMatPath = @"E:\HuiTask\Улучшенный Genshin Impact\сопоставление карт\sift.mat";
            ...
            sw.Stop();
            Debug.WriteLine($"большая картаkpкропотливый：{sw.ElapsedMilliseconds}ms.");
            sw.Restart();
            sift.DetectAndCompute(matTo, null, out keyPointsTo, matToRet);
            sw.Stop();
            Debug.WriteLine($"шаблонkpкропотливый：{sw.ElapsedMilliseconds}ms.");
            sw.Restart();
        }
        ...
        using (var bfMatcher = new OpenCvSharp.BFMatcher())
        {
            ...
            sw.Stop();
            Debug.WriteLine($"bfMatcherкропотливый：{sw.ElapsedMilliseconds}ms.");
            sw.Restart();
            ...
            sw.Stop();
            Debug.WriteLine($"FindHomographyкропотливый：{sw.ElapsedMilliseconds}ms.");
            sw.Restart();
            ...
            sw.Stop();
            Debug.WriteLine($"Рисуноккропотливый：{sw.ElapsedMilliseconds}ms.");
            sw.Restart();
            return outMat;
        }
    }*/
    ...
    public static Mat MatchPicBySurf(Mat matSrc, Mat matTo, double threshold = 400)
    {
        ...
        using (var flnMatcher = new FlannBasedMatcher())
        {
            ...
            sw.Stop();
            Debug.WriteLine($"flnMatcherкропотливый：{sw.ElapsedMilliseconds}ms.");
            sw.Restart();
            ...
            if (pSrc.Count > 0 && pDst.Count > 0)
            {
                ...
                // for (int i = 0; i < 4; i++)
                // {
                //     /* функция иperspectiveTransformТакой же
            ...
            if (outMask.Rows > 10)
            {
                Debug.WriteLine($"использовалRansacрезультат");
                byte[] maskBytes = new byte[outMask.Rows * outMask.Cols];
                outMask.GetArray(out maskBytes);
                Cv2.DrawMatches(matSrc, keyPointsSrc, matTo, keyPointsTo, goodMatches, outMat, matchesMask: maskBytes, flags: DrawMatchesFlags.NotDrawSinglePoints);
            }
