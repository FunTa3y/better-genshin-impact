...
public class KeyPointFeatureBlockHelper
{
    public static KeyPointFeatureBlock[][] SplitFeatures(Size originalImage, int rows, int cols, KeyPoint[] keyPoints, Mat matches)
    {
        ...
        // Перебрать каждый функциональный блок，Вычислить дескриптор
        for (int i = 0; i < rows; i++)
        {
    ...
    /// <summary>
    /// Копировать построчноmatchesприезжатьdescriptor
    /// </summary>
    /// <param name="keyPointIndexList">Запишите, какие строки необходимо скопировать</param>
    /// <param name="descriptor">Копировать результаты</param>
    /// <param name="matches">Необработанные данные</param>
